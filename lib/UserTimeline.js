...
class UserTimeline extends EventEmitter {
  ...
  async getTimeline(name, count) {
    const data = await this.T.get('/statuses/user_timeline',
      { screen_name: name, count, tweet_mode:'extended' });
    return data;
  }
  ...
  async getRangeIds(user, count) {
    ...
    console.log(`Greatest Tweet ID ${greatest.length}`);

    if (lowest.length > 0 && greatest.length > 0) {
      console.log(`The user ${user} exist. Getting timeline range 
            from ${lowest} to ${greatest}`);
      return this.getTimelineRange(user, count, lowest[0].id_str, greatest[0].id_str);
    }
    console.log(`First time getting from ${user}`);
  ...
  async iterateTweets(user, id, count, action) {
    ...
    while (result !== 0) {
      ...
      try{
        ...
        if ( result - 1  > 0 ){
          compareId = data.data[result - 1].id_str;
        }else{
          compareId = lastId;
        }
