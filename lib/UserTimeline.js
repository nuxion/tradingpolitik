  ...
  async iterateTweets(user, id, count, action) {
    ...
    while (result !== 0) {
      ...
      try{
        ...
        console.log(`Begin: lastId: ${lastId}, compareId: ${compareId}, i: ${i}`);
        if (compareId === lastId || result === 0) {
          console.log('Equals Id');
          console.log(`lastId: ${lastId}, compareId: ${compareId}, i: ${i}`);
					result = 0;
				} else {
					await Promise.all(data.data.map(this.control.saveData));
