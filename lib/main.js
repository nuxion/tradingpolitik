...
async function getPaginate(user, count, Model){
...
async function cursoring(user, count, Model) {
  ...
  var i = 0;
  while(result != 0){
    data = await getPaginate(user, count, Model);
    result = data.data.length;
    i++;
    console.log('Una vuelta: '+i);
...
/*getPaginate(name, count, TimelineModel).then((data)=>{
